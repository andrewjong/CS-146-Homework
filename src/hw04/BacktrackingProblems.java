...
public class BacktrackingProblems {
	 ...
	 * with the additional condition that all elements that make up the sum must be odd? An empty set of integers is
	 * considered to have a sum of 0, and have only odd integers (a vacuously true statement) Solve this problem without
	 * using any loops.
	 * @return true if such a subset exists
	 */
	public static boolean oddSubsetSum(List<Integer> list, int target) {
		...
		boolean numberIsOdd = list.get(0) % 2 != 0;
		List<Integer> restOfTheList = list.subList(1, list.size());
		boolean canMakeSubsetUsingThisElement = false;
		if (numberIsOdd) {
			canMakeSubsetUsingThisElement = oddSubsetSum(restOfTheList, target - list.get(0));
		}
		boolean canMakeSubsetWithoutThisElement = oddSubsetSum(restOfTheList, target);

		return canMakeSubsetUsingThisElement || canMakeSubsetWithoutThisElement;
	}
	...
	public static boolean subsetOf8Sum(List<Integer> list, int target) {
		return subsetOfNSum(list, target, 8, 8);
	}

	public static boolean subsetOfNSum(List<Integer> list, int target, int n, int digitsNeeded) {
		return false;
	}
	 ...
	 * Solve this problem without using any loops.
	 * <p>
	 * Hint 1: sum is to subtraction as product is to ... Hint 2: what happens to the order of three values 0 < a < b <
	 * c if you divide them all by the same negative number? what about if you divide by the same number between 0 and
	 * 1? Hint 3: use inequalities (<, >, <=, >=) to compare doubles, do not use equality (==).
	 * @return true if such a subset exists
	 */
